(()=>{
  'use strict';
  const label=(el)=>{const id=el.id?`#${el.id}`:'';const cls=[...el.classList].slice(0,3).map(x=>`.${x}`).join('');const name=el.getAttribute('name')?`[name=${el.getAttribute('name')}]`:'';return `${el.tagName.toLowerCase()}${id}${cls}${name}`};
  const rectCheck=(sel,w,failures,required=true)=>{
    ...
    if(r.left<-3||r.right>w+3)failures.push(`${sel} outside ${Math.round(r.left)}..${Math.round(r.right)} @${w}`);
  };
  const publicChecks=(w,failures)=>{
    ...
    document.querySelectorAll('input,select,textarea,.btn').forEach(el=>{const r=el.getBoundingClientRect(),cs=getComputedStyle(el);if(cs.display==='none'||r.width===0)return;if(r.width>w+2||r.right>w+3||r.left<-3)failures.push(`${label(el)} overflow w=${Math.round(r.width)} x=${Math.round(r.left)}..${Math.round(r.right)} @${w}`)});
    const h1=document.querySelector('.hero h1');if(h1&&h1.getBoundingClientRect().width>w+3)failures.push('hero heading overflow');
    if(w<=960){const toggle=document.querySelector('.titanium-mobile-toggle'),nav=document.querySelector('.nav-links');if(!toggle)failures.push('mobile toggle missing');else{toggle.click();const r=nav?.getBoundingClientRect();if(!nav||getComputedStyle(nav).pointerEvents==='none')failures.push('mobile menu did not open');if(r&&(r.left<-3||r.right>w+3||r.width>w+3))failures.push(`mobile menu overflow ${Math.round(r.left)}..${Math.round(r.right)} @${w}`);toggle.click()}}
  ...
  const finish=()=>{
    ...
    requestAnimationFrame(()=>{
      ...
      body.dataset.titaniumQa=failures.length?'fail':'ok';body.dataset.titaniumQaWidth=String(w);body.dataset.titaniumQaFailures=failures.join(' | ').slice(0,1800);
