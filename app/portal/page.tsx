...
export default function PortalPage(){
  ...
  async function logout(){const supabase=createClient();await supabase.auth.signOut();location.href=withBasePath('/portal')}
  ...
  if(!profile)return <main className="grid min-h-screen place-items-center px-4"><div className="w-full max-w-xl rounded-[32px] border border-slate-200 bg-white p-8 text-center shadow-soft md:p-12"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl border border-blue-100 bg-white p-1 shadow-card"><img src={withBasePath('/nexora-mark.svg')} alt="NEXORA" className="h-11 w-11"/></div><div className="mt-7 text-xs font-black tracking-[.18em] text-[#0071e3]">NEXORA PORTAL</div><h1 className="mt-4 text-3xl font-black">بوابة العملاء</h1><p className="mt-4 leading-8 text-slate-500">سجّل الدخول بحساب العميل المعتمد لمتابعة مشاريعك وملفاتك وتحديثاتك.</p><div className="mt-7 flex flex-wrap justify-center gap-2"><Link href="/portal/login" className="btn-primary">تسجيل دخول العميل</Link><Link href="/portal/activate" className="btn-secondary">تفعيل حساب جديد</Link></div></div></main>

  return <main className="min-h-screen bg-[#f6f7f9]">
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="container-shell flex min-h-[76px] items-center justify-between"><Link href="/" className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl border border-blue-100 bg-white p-1 shadow-card"><img src={withBasePath('/nexora-mark.svg')} alt="NEXORA" className="h-8 w-8"/></span><span className="font-[var(--font-inter)] text-sm font-black tracking-[.16em]">NEXORA <b className="text-[#0071e3]">PORTAL</b></span></Link><div className="flex items-center gap-3"><Link href="/portal/messages" className="hidden h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-black text-slate-600 sm:flex"><MessageSquareMore size={16}/> الرسائل</Link><div className="hidden text-left sm:block"><strong className="block text-sm">{profile.full_name||profile.email}</strong><span className="text-xs font-bold text-slate-400">Client</span></div><button onClick={logout} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500"><LogOut size={17}/></button></div></div></header>
    <div className="container-shell py-8 md:py-12">
      <section className="rounded-[30px] border border-slate-200 bg-white p-7 shadow-card md:p-10"><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><div className="eyebrow">CLIENT WORKSPACE</div><h1 className="mt-5 text-3xl font-black md:text-4xl">أهلاً {profile.full_name||''}</h1><p className="mt-3 max-w-2xl leading-8 text-slate-500">هنا تشوف فقط المعلومات المخصصة لك: المشاريع، الملفات، المهام، العروض وطلبات التغيير.</p></div><div className="flex flex-wrap gap-2"><Link href="/portal/messages" className="btn-secondary"><MessageSquareMore size={17}/> رسائل المشاريع</Link>{projects.length>0&&<button onClick={()=>setOpenChange(true)} className="btn-primary"><Plus size={17}/> طلب تغيير</button>}</div></div></section>
