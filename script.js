(()=>{
  ...
  const polishShowcase=()=>{
    ...
    cards.forEach((card,i)=>{const p=card.querySelector('p');if(p&&copy[i]){p.dataset.ar=copy[i].p;if((document.documentElement.lang||'ar')==='ar')p.textContent=copy[i].p;}});
  };
  ...
  const normalizeRoutes=(root=document)=>{
    ...
    root.querySelectorAll?.('img[src="assets/dashboard-showcase.jpg"],img[src$="/assets/dashboard-showcase.jpg"]').forEach(img=>{img.src=`${base}/assets/nexora-platform.svg`;img.alt='NEXORA TITANIUM operations interface preview';});
    polishShowcase();
  };

  document.addEventListener('click',e=>{
    const a=e.target.closest?.('a[href]');if(!a)return;
    const raw=(a.getAttribute('href')||'').trim(),clean=raw.replace(/^https?:\/\/[^/]+/,'');
    if(clean==='/dashboard/'||clean==='/nexora-ai/dashboard/'||clean==='dashboard/'||clean==='./dashboard/'){e.preventDefault();location.href=appPath('/dashboard/');return;}
    if(clean==='/portal/'||clean==='/nexora-ai/portal/'||clean==='portal/'||clean==='./portal/'){e.preventDefault();location.href=appPath('/portal/');}
  },true);

  normalizeRoutes();
  const observer=new MutationObserver(records=>records.forEach(r=>r.addedNodes.forEach(n=>{if(n.nodeType===1)normalizeRoutes(n)})));
  ...
  load('./base-script.js?v=titanium-20260909-1')
    .then(()=>load('./premium-home.js?v=titanium-20260909-1'))
    .then(()=>load('./route-guard.js?v=titanium-20260909-1'))
    .then(()=>load('./titanium.js?v=titanium-20260909-1'))
